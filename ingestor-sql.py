import psycopg2 # Interage com postgre, executa DML
from faker import Faker # Gera dados Aleatórios
import random # Gera dados aleatórios
from dotenv import load_dotenv # Carrega variáveis de ambiente para dentro da  aplicação ( Faz o export das variaveis de ambientes e pega os valores)
import os

def createTables(cur): # cur = cursor
    try:
...
def randomData(cur, conn, numClients):
    try:
        ...
        for i in range(numClients):
            name = fake.name()         # Inserindo os dados aleatórios
            email = fake.email()
            cur.execute("INSERT INTO Customers (name, email) VALUES (%s, %s)", (name, email))
            customers.append(name)

        conn.commit() # Salva os dados direto do banco 
    ...
    try:
        ...
        for i in range(len(products)): # itera sobre a lista e faz o insert
            name = products[i]
            category = categories[i % len(categories)]
            price = round(random.uniform(50, 1000), 2) # pega o preço d eforma aleatória
            cur.execute("INSERT INTO Products (name, category, price) VALUES (%s, %s, %s)", (name, category, price))
        conn.commit() # Salvando os dados no banco
    except Exception as e:
    ...
    try:
        ...
        for i in range(len(products)):
            customer_id = random.randint(1, len(customers)) # pega o id aleatório do customer
            product_id = random.randint(1, len(products)) # pega o id aleatório do produto
            quantity = random.randint(1, 5) # pega a quantidade
...
fake = Faker(locale='pt_BR') # Vai gerar dados aleatórios em portugues. 

# Carrega as variáveis de ambiente do arquivo .env
load_dotenv() # Usado para carregar tudo que tem arquivo .env para memória como se fosse um export. 
...
cur = conn.cursor()
createTables(cur)
randomData(cur, conn, numClients) # inclui na tabela dados aleatórios
